...
pub struct PlayerConnection {
    pub input_source: Arc<Mutex<GameServerPlayerInputSource>>
}
...
pub struct GameServer {
    table: Table,
    players: Mutex<HashMap<PlayerId, Arc<Mutex<GameServerPlayerInputSource>>>>,
    log_update_channel_r: fold_channel::Receiver<Vec<PokerGlobalViewDiff<PlayerId>>>,
    bots: Mutex<Vec<Arc<Mutex<BotAlwaysCallInputSource>>>>,
...
pub struct GameServerPlayerInputSource {
    ...
    bet_os: Option<oneshot::Sender<BetResp>>
}
...
impl GameServerPlayerInputSource {
    ...
    fn server_player(&self) -> ServerPlayer {
        ...
        let action_requested = if self.bet_os.is_some() {
            self.action_rx.borrow().clone()
        } else {
            None
        };
...
impl GameServer {
    ...
    pub async fn notify_player<'a>(&'a self, player_id: PlayerId, start_from: usize, known_action_requested: Option<ServerActionRequest>) -> (Vec<PokerLogUpdate>, ServerPlayer) {
        ...
        let mut action_rx = player.lock().unwrap().action_tx.subscribe();
        ...
        loop {
            {
                let mut source = player.lock().unwrap();
    ...
    pub fn get_player(&self, player_id: PlayerId) -> Option<Arc<Mutex<GameServerPlayerInputSource>>> {
        use std::collections::hash_map::Entry::*;
        let mut players = self.players.lock().unwrap();
        match players.entry(player_id.clone()) {
            Occupied(e) => Some(e.get().clone()),
            Vacant(v) => {
                let new_player = Arc::new(Mutex::new(GameServerPlayerInputSource::new()));
        ...
        match (req.method(), req.uri().path()) {
            (&Method::GET, "/game") => {
                if let Some(player_id) = player_from_params(&params) {
                    if let Some(input_source) = self.get_player(player_id) {
                        let player = input_source.lock().unwrap().server_player();
            ...
            (&Method::POST, "/add_bot") => {
                let (player_id, new_bot) = {
                    ...
                    let bot = Arc::new(Mutex::new(BotAlwaysCallInputSource::new()));
                    bots.push(bot.clone());
                ...
                if let Some(player_id) = player_from_params(&params) {
                    if let Some(conn) = self.get_player(player_id) {
                        if let Ok(resp) = serde_json::from_slice::<BetResp>(&hyper::body::to_bytes(req.into_body()).await.unwrap()) {
                            let mut input_source = conn.lock().unwrap();
                            let player = input_source.server_player();
                            if input_source.bet_os.is_some() {
                                    ...
                                    match resp {
                                        BetResp::Bet(bet) => {
                                            if player.viewstate.as_ref().map(|s| s.valid_bet(min_bet, call_amount, bet, s.role).is_ok()).unwrap_or(false) {
                                                if let Some(channel) = input_source.bet_os.take() {
                                                    channel.send(resp);
                                                    *response.status_mut() = StatusCode::OK;
                                                }
                                            }
